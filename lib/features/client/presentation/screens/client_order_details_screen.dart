...
class _ClientOrderDetailsScreenState extends State<ClientOrderDetailsScreen> {
  ...
  Future<void> _load({bool silent = false}) async {
    ...
    try {
      ...
      final order =
          orderMap == null ? null : OrderModel.fromMap(orderMap);
      final items = itemsMap.map(OrderItemModel.fromMap).toList();
      ...
      body: _loading
          ...
          : order == null
              ...
              : ListView(
                  ...
                  children: [
                    _row('الحالة', _statusText(order.status)),
                    _row('الهاتف', order.phone),
                    _row(
                      'الإجمالي',
                      '${_money(order.totalCents)} جنيه',
                    ),
                    _row(
                      'المدفوع',
                      '${_money(order.paidCents)} جنيه',
                    ),
                    _row(
                      'ملاحظات الإدارة',
                      order.adminNote,
                    ...
                    _row(
                      'موعد الصيانة',
                      _fmtDate(order.scheduledAt),
                    ),
                    _row(
                      'وقت الإنهاء',
                      _fmtDate(order.completedAt),
                    ),
                    _row(
                      'تاريخ الإنشاء',
                    ...
                    ..._items.map(
                      (item) => Card(
                        child: ListTile(
                          ...
                          subtitle: Text(
                            'الكمية: ${item.qty} • السعر: ${_money(item.unitPriceCents)} جنيه',
                          ),
                        ),
                      ),
                    ),
